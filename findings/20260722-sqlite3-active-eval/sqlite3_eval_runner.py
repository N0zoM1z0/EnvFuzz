...
def run_one(repo, root, config, seed):
    out_dir = root / "runs" / config["name"] / str(seed)
    out_dir.mkdir(parents=True, exist_ok=False)
    shutil.copy2(root / "recordings/base-exit/RECORD.pcap.gz", out_dir / "RECORD.pcap.gz")
...
def main():
    ...
    root = repo / "findings/20260722-sqlite3-active-eval"
    configs = [
        {"name": "nograph"},
        {"name": "filtered_active_graph", "graph": root / "tmp/sqlite3-filtered-active.graph.json"},
        {
            "name": "tty_action_only_state_graph",
            "graph": root / "tmp/sqlite3-tty-action-only-state.graph.json",
        },
        {
            "name": "tty_action_state_graph",
            "graph": root / "tmp/sqlite3-tty-action-state.graph.json",
        },
    ]
    seeds = list(range(7301, 7311))

    rows = []
    for config in configs:
        for seed in seeds:
            row = run_one(repo, root, config, seed)
            rows.append(row)
            print(
                "%-24s seed=%s rc=%s outs=%s/%s patches=%s graph=%s "
    ...
    jsonl_path = root / "sqlite3-results.jsonl"
    csv_path = root / "sqlite3-results.csv"
    summary_path = root / "sqlite3-summary.json"
