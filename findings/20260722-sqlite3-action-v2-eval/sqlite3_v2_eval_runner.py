...
def main():
    ...
    root = repo / "findings/20260722-sqlite3-action-v2-eval"
    ...
    configs = [
        ...
        {
            "name": "tty_action_single_state_graph",
            "graph": root / "tmp/sqlite3-tty-action-single-state.graph.json",
        },
        {
            "name": "tty_action_only_single_state_graph",
            "graph": root / "tmp/sqlite3-tty-action-only-single-state.graph.json",
        },
    ]
    ...
    for config in configs:
        for seed in seeds:
            ...
            rows.append(row)
            print(
                "%-36s seed=%s rc=%s outs=%s/%s patches=%s graph=%s "
    ...
    jsonl_path = root / "sqlite3-v2-results.jsonl"
    csv_path = root / "sqlite3-v2-results.csv"
    summary_path = root / "sqlite3-v2-summary.json"
