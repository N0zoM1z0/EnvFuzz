...
def write_outputs(root, rows):
    jsonl_path = root / "nano91-tty-action-results.jsonl"
    csv_path = root / "nano91-tty-action-results.csv"
    summary_path = root / "nano91-tty-action-summary.json"
...
def load_existing_rows(root):
    path = root / "nano91-tty-action-results.jsonl"
    if not path.exists():
        return []
    rows = []
...
def main():
    ...
    root = repo / "findings/20260722-nano-tty-action-eval"
    filtered_graph = (
        repo
        / "findings/20260722-nano-active-eval/tmp/nano91-filtered-active.graph.json"
    )
    action_graph = root / "tmp/nano91-tty-action-state.graph.json"
    configs = [
        {"name": "nograph"},
        {"name": "filtered_active_graph", "graph": filtered_graph},
        {"name": "tty_action_state_graph", "graph": action_graph},
        {"name": "tty_action_budget1_graph", "graph": action_graph},
    ]
    seeds = list(range(6401, 6411))
    max_execs = 10000
