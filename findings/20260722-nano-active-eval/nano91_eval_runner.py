...
def main():
    ...
    root = repo / "findings/20260722-nano-active-eval"
    graph = root / "tmp/nano91-filtered-active.graph.json"
    configs = [
        {"name": "improved_nograph"},
        {"name": "improved_filtered_active_graph", "graph": graph},
    ]
    ...
    for config in configs:
        for seed in seeds:
            ...
            rows.append(row)
            print(
                "%-31s seed=%s rc=%s outs=%s/%s patches=%s graph=%s "
    ...
    jsonl_path = root / "nano91-results.jsonl"
    csv_path = root / "nano91-results.csv"
    summary_path = root / "nano91-summary.json"
