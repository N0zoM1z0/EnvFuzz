...
def run_one(repo, root, focused_cwd, source_recording, config, seed):
    out_dir = root / "runs" / "focused-eval" / config["name"] / str(seed)
    out_dir.mkdir(parents=True, exist_ok=False)
    shutil.copy2(source_recording, out_dir / "RECORD.pcap.gz")
...
def main():
    ...
    root = repo / "findings/20260723-wsl-gui-calc-active-eval"
    focused_cwd = root / "focused-cwd"
    source_recording = root / "recordings/basic-arithmetic/RECORD.pcap.gz"
    configs = [
        {"name": "nograph"},
        {
            "name": "wayland_only_graph",
            "graph": root / "tmp/calc-wayland-only.graph.json",
        },
        {
            "name": "wayland_bus_graph",
            "graph": root / "tmp/calc-wayland-active.graph.json",
        },
    ]
    seeds = list(range(8201, 8206))

    rows = []
    for config in configs:
        for seed in seeds:
            row = run_one(repo, root, focused_cwd, source_recording, config, seed)
            rows.append(row)
            print(
                "%-20s seed=%s rc=%s outs=%s/%s patches=%s graph=%s "
    ...
    jsonl_path = root / "calc-focused-results.jsonl"
    csv_path = root / "calc-focused-results.csv"
    summary_path = root / "calc-focused-summary.json"
